/*----------------------------------------------------------------
    ...
    文件名：LimeMessageQueue.cs
    文件功能描述：LimeMessageQueue消息列队
...
namespace Lime.Weixin.MessageQueue
{
    ...
    /// 消息列队
    /// </summary>
    public class LimeMessageQueue
    {
        /// <summary>
        /// 列队数据集合
        /// </summary>
        private static Dictionary<string, LimeMessageQueueItem> MessageQueueDictionary = new Dictionary<string, LimeMessageQueueItem>(StringComparer.OrdinalIgnoreCase);
        ...
        /// <summary>
        /// 获取LimeMessageQueueItem
        /// </summary>
        /// <param name="key"></param>
        /// <returns></returns>
        public LimeMessageQueueItem GetItem(string key)
        {
            lock (MessageQueueSyncLock)
            {
        ...
        public LimeMessageQueueItem Add(string key, Action action)
        {
            lock (MessageQueueSyncLock)
            {
                //if (!MessageQueueDictionary.ContainsKey(key))
                ...
                var mqItem = new LimeMessageQueueItem(key, action);
